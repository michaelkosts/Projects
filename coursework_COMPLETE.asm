...
move_up:
    subi $t1, $t1, 1   				# Move up row--
    j update_position

move_down:
    addi $t1, $t1, 1   				# Move down row++
    j update_position

move_left:
    subi $t2, $t2, 1   				# Move left column--
    j update_position

move_right:
    addi $t2, $t2, 1   				# Move right column++
    j update_position
...
update_position:
    ...
    # Check if player has collected the reward.
...
# New reward generation loop:
pick_new_reward:
pick_new_reward_loop:
    # Generate new reward row:
    # Generate a number between 0 and 5 (exclusive 5) then add 1 to obtain a number between 1 and 5.
    li $v0, 42
    li $a0, 0         # Lower bound: 0
    li $a1, 5         # Upper bound (exclusive): 5
    syscall
    addi $s0, $a0, 1  # s0 now is in the range 1-5

    # Generate new reward column:
    # Generate a number between 0 and 7 (exclusive 7) then add 1 to obtain a number between 1 and 7.
    li $v0, 42
    li $a0, 0         # Lower bound: 0
    li $a1, 7         # Upper bound (exclusive): 7
    syscall
    addi $s1, $a0, 1  # s1 now is in the range 1-7

    # If new reward location is exactly where the player is, regenerate.
    beq $s0, $t1, pick_new_reward_loop
    beq $s1, $t2, pick_new_reward_loop

    # Once a free spot is found, update the display.
reward_okay:
    jal display_game            # Update display with new reward position.
    j game_loop                 # Continue the game loop.
...
# Prints "Score: {counter}" and the grid (walls '#', player 'P', reward 'R')
...
print_decimal_output_loop:
    ...
    bgtz $t9, print_decimal_output_loop	# if digit counter > 0, loop again (this counter is now useful, as it prints out all our stored ASCII chars and decrements stack)
    lw $t3, 0($sp)
    lw $ra, 4($sp)
